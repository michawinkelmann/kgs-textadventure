...
Tipp: Nutze die Vorschläge im Kontext‑Kasten rechts.`);
  ...
  function renderMap(){
    ...
    // Helper: is neighbor from current?
    ...
    bg.setAttribute("fill", "rgba(255,255,255,0.00)");
    svg.appendChild(bg);
    ...
    for (const {a, b} of edges.values()){
      ...
      // Highlight if edge touches current
      const touchesCur = (a === curId || b === curId);
      if (touchesCur) { opacity = "0.55"; stroke = "rgba(59,130,246,0.50)"; }

      // If neighbor is locked from current, draw dashed + amber
      ...
      line.setAttribute("stroke-width", touchesCur ? "6" : "4");
    ...
    for (const [locId, n] of Object.entries(nodes)){
      ...
      const r = isHere ? 16 : (isNeighbor ? 13 : 11);
      ...
      label.setAttribute("fill", "rgba(229,231,235,0.78)");
      label.setAttribute("font-size", "18");
      label.setAttribute("font-family", "system-ui");
      label.textContent = n.label || locId;
      g.appendChild(label);

      // Click behavior: move if direct & unlocked, else show route
      g.addEventListener("click", () => {
        ...
        // Convert path into command chain
        const cmds = [];
...
// ---------- Rendering ----------
  ...
  function load(){
    try {
      ...
      state.inventory = Array.isArray(data.inventory) ? data.inventory : state.inventory;
